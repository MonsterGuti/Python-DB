...
def show_highest_rated_art():
    highest_rated_art_gallery = ArtworkGallery.objects.all().order_by('-rating').first()
    return (f"{highest_rated_art_gallery.art_name} is the highest-rated art with a"
            f" {highest_rated_art_gallery.rating} rating!")
...
def grand_chess_title_regular_player():
    ChessPlayer.objects.filter(rating__gte=0, rating__lte=2100).update(title='regular player')
