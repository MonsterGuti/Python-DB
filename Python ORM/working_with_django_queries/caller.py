...
def find_authors_nationalities():
    authors = Author.objects.all().exclude(nationality=None)
    result = ''
    for author in authors:
        result += f'{author.first_name} {author.last_name} is {author.nationality}\n'
    return result.strip()
...
def order_books_by_year():
    ...
    result = ''
    for book in books:
        result += f'{book.publication_year} year: {book.title} by {book.author}\n'
    return result.strip()
...
def filter_authors_by_nationalities(nationality):
    ...
    result = [
        a.biography if a.biography else f"{a.first_name} {a.last_name}"
...
def change_reviewer_name(reviewer, new_name):
    Review.objects.filter(reviewer_name=reviewer).update(reviewer_name=new_name)
